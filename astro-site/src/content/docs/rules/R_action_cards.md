...
2. Players can draw action cards by resolving the primary and secondary abilities of the <i>Politics</i> strategy card.
...
4. Each player’s hand can have a maximum of <b>seven action cards</b>. If a player ever has more than seven action cards, that player must choose seven cards to keep and discard the rest.
...
6. The first paragraph of each action card is presented in bold text and describes the timing of when that card’s ability can be resolved.
    ...
    2. Multiple action cards with the <b>same</b> name cannot be played during a single timing window to affect the same units or game effect. Canceled cards are not treated as being played.
...
2. The number of action cards in a player’s hand is public information.
3. A player may choose to reveal any or all of the action cards in their hand, if they so choose.
4. If the action card deck is depleted, immediately shuffle the discard pile to form a new action card deck.
5. During the action phase, action cards are dealt out in initiative order.
 
    1. If a player has the <i>Neural Motivator</i> technology, they will draw both their action cards before the next player in order draws an action card.
    2. The Yssaril player will draw all of their action cards, and discard for their Scheming ability, before the next player in order draws an action card.
 
6. If a player’s action card is cancelled, usually via a <i>Sabotage</i> action card, then if that player has a second copy of that action card, they may play it.
7. A player cannot target themselves with offensive action cards, such as <i>Lucky Shot</i>.
